...
The number of askers however, greatly increases the execution time. Because readers cause all other threads to block,
there is more competition over the shared resource.

You can see the exact output for each individual test run, for min_word and max_word in their respective .txt files, i.e
...
***(we've altered emerging.go to call min_word instead of max_word, and we store these results in the min_results directory)***
